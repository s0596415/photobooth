...
const backgrounds = [
    ...
    { name: "Schlittschuhe", image: "bilder/5.png" }, //{ name: 'Tannenbaum', image: 'bilder/6.png' },
...
function updatePhotoCounter(initial = false) {
    ...
    if (initial) {
        ...
        const current =
            state.photos.length < layout.count
...
function loadImage(src) {
    return new Promise((resolve, reject) => {
        ...
        img.onerror = (err) =>
            reject(
                new Error(`Bild konnte nicht geladen werden: ${src}`, { cause: err }),
            );
        img.src = src;
    });
}
...
async function startCamera() {
    try {
        ...
        video
            .play()
            .catch((e) =>
                console.error("Video Playback Startfehler (wegen Autoplay-Regeln):", e),
            );
    } catch (err) {
        console.error("Kamerafehler:", err);
        alert("Kamerazugriff verweigert. Bitte erlaube den Kamerazugriff.");
...
async function generatePhotostrip(canvas) {
    ...
    canvas.height =
        layout.rows * photoHeight + (layout.rows + 1) * padding + footerHeight;
    ...
    const photoBlockWidth =
        layout.cols * photoWidth + (layout.cols - 1) * padding;
    const photoBlockHeight =
        layout.rows * photoHeight + (layout.rows - 1) * padding;
...
function setupCustomization() {
    ...
    colorGrid.innerHTML = ""; // --- HINTERGRUNDBILDER ---

    backgrounds.forEach((bg, index) => {
        const div = document.createElement("div");
        const isSelected =
            (!state.backgroundImage && index === 0) ||
        ...
        div.addEventListener("click", async () => {
            document
                .querySelectorAll(".bg-option, .color-option")
...
document.getElementById("start-btn").addEventListener("click", () => {
    ...
    if (state.selectedLayout) {
        appStats.layoutsUsed[state.selectedLayout].count += 1;
    }
});
...
document.getElementById("capture-btn").addEventListener("click", async () => {
    ...
    for (let i = 0; i < layout.count; i++) {
        ...
        if (isMultiPhotoLayout && !isLastPhoto) {
            countdownOverlay.textContent =
                "Super! Mach dich bereit für das nächste Foto...";
...
document.getElementById("customize-next-btn")
    .addEventListener("click", async () => {
        showScreen("download");

        const qrTarget = document.getElementById("qr-code-target");
        const downloadBtn = document.getElementById("download-btn");
        const qrBtn = document.getElementById("qr-btn"); // Buttons verstecken, bis alles fertig ist

        downloadBtn.style.display = "none";
        qrBtn.style.display = "none"; // Warten, bis generatePhotostrip() FERTIG ist

        await generatePhotostrip(finalCanvas); // Ab hier ist der finalCanvas garantiert voll gezeichnet!
        // Buttons jetzt anzeigen
        downloadBtn.style.display = "block";
        qrBtn.style.display = "block"; // --- START: Upload-Logik ---
        ...
        try {
            ...
            if (!result.url)
                throw new Error("Server hat keine gültige URL zurückgegeben.");
...
function logAccess(username, status, details = "") {
    const logs = JSON.parse(
        localStorage.getItem("photobooth_access_logs") || "[]",
    );
...
function downloadLogsCSV() {
    ...
    let csvContent =
        "data:text/csv;charset=utf-8,Datum & Uhrzeit;Benutzer / Bereich;Status;Details\n";
    logs.forEach((row) => {
    ...
    link.setAttribute(
        "download",
        `photobooth_access_logs_${new Date().toISOString().slice(0, 10)}.csv`,
    );
...
if (loginForm) {
    loginForm.addEventListener("submit", (event) => {
        ...
        if (user === "admin" && bcrypt.compareSync(pass, ADMIN_HASH)) {
            ...
            currentUser = "admin";
            document.getElementById("admin-settings-btn").style.display =
                "inline-block";
    ...
    function updateGuestLayouts() {
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            const checkbox = document.querySelector(
                `.layout-toggle-cb[data-layout="${i}"]`,
            );
...
if (customizeNextBtn) {
    customizeNextBtn.addEventListener("click", () => {
        ...
        drawHistory = [
            ctxDraw.getImageData(0, 0, drawingCanvas.width, drawingCanvas.height),
        ];
    });
}
...
if (!blob) {
  ...
  try {
    ...
    //appStats.totalPhotos += 1; //online zähler
...
document
    .getElementById("gallery-password-form")
...
async function loadGallery() {
    ...
    try {
        ...
        if (photos.length === 0) {
            galleryGrid.innerHTML =
                '<p id="empty-gallery-msg" style="grid-column: 1 / -1; text-align: center; color: #666;">Noch keine Bilder vorhanden.</p>';
            return;
        }
...
/* =========================================
   DASHBOARD & STATISTIKEN LOGIK
   ========================================= */

// Unser Statistik-Speicher (im Arbeitsspeicher)
let appStats = {
    totalPhotos: 0,
    failedUploads: 0,
    layoutsUsed: {
        1: { name: "1×3 Classic", count: 0 },
        2: { name: "1×4 Strip", count: 0 },
        3: { name: "Instax Quadrat", count: 0 },
        4: { name: "2x2 Grid", count: 0 },
    },
};

// Diese Funktion berechnet das beliebteste Layout
function getTopLayout() {
    let topName = "-";
    let maxCount = 0;

    for (let key in appStats.layoutsUsed) {
        if (appStats.layoutsUsed[key].count > maxCount) {
            maxCount = appStats.layoutsUsed[key].count;
            topName = appStats.layoutsUsed[key].name;
        }
    }
    return maxCount === 0 ? "Noch keins" : topName;
}

// Aktualisiert die Anzeige im Dashboard
function updateDashboardUI() {
    document.getElementById("stat-total-photos").innerText = appStats.totalPhotos;
    document.getElementById("stat-top-layout").innerText = getTopLayout();
    document.getElementById('stat-failed-uploads').innerText = appStats.failedUploads;
}

// Event-Listener für das Dashboard Modal
const dashboardModal = document.getElementById("dashboard-modal");
const dashboardBtn = document.getElementById("admin-dashboard-btn");
const closeDashboardBtn = document.getElementById("close-dashboard-btn");

dashboardBtn.addEventListener("click", () => {
    updateDashboardUI(); // Werte updaten, bevor wir es anzeigen
